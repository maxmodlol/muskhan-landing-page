    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              سيتم حذف هذا السجل نهائياً مع صورة إثبات الدفع من التخزين. لا يمكن التراجع عن هذا الإجراء.
            </AlertDialogDescription>
          </AlertDialogHeader>
          {deleteDialogError ? (
            <p
      ...
      <header className="flex flex-col gap-4 border-b border-earth/10 pb-6 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-earth/70">مهرجان المسخن — لوحة التحكم</p>
        </div>
        ...
        <>
          <div className="mt-8 grid gap-4 sm:grid-cols-3">
            <div className="rounded-2xl border border-earth/10 bg-cream/90 p-5 shadow-sm ring-1 ring-gold/10">
              <p className="text-sm font-medium text-earth/70">إجمالي الحجوزات</p>
          ...
          {rows.length === 0 ? (
            <p className="mt-12 rounded-2xl border border-dashed border-earth/20 bg-parchment/80 py-16 text-center text-earth/65">
              لا توجد حجوزات بعد.
            </p>
          ) : (
            <div className="mt-8 overflow-x-auto rounded-2xl border border-earth/10 bg-cream/50 shadow-sm ring-1 ring-earth/5">
              <table className="w-full min-w-[720px] border-collapse text-right text-sm">
                <thead>
                  <tr className="border-b border-earth/15 bg-parchment/90">
                    <th className="px-3 py-3 font-semibold text-olive sm:px-4">الاسم</th>
                    <th className="px-3 py-3 font-semibold text-olive sm:px-4">الهاتف</th>
                    <th className="px-3 py-3 font-semibold text-olive sm:px-4">الضيوف</th>
                    <th className="px-3 py-3 font-semibold text-olive sm:px-4">ملاحظة</th>
                    <th className="px-3 py-3 font-semibold text-olive sm:px-4">إثبات الدفع</th>
                    <th className="px-3 py-3 font-semibold text-olive sm:px-4">التاريخ</th>
                    <th className="px-3 py-3 font-semibold text-olive sm:px-4">إجراءات</th>
                  </tr>
                </thead>
                <tbody>
                  {rows.map((r) => (
                    <tr key={r.id} className="border-b border-earth/10 last:border-0">
                      ...
                      <td className="px-3 py-3 align-top sm:px-4">
                        {r.payment_proof_view_url ? (
                          ...
                          <span className="text-earth/60" title="تعذّر إنشاء رابط مؤقّت">
                            غير متاح
                          </span>
                        ) : (
                          "—"
